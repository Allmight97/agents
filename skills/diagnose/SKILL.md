...
Phase 1 is done when you can name one command you have already run at least once (paste invocation and output) that is:

- **Red-capable**: it drives the actual bug path and asserts the user's exact symptom, not merely "does not crash".
- **Deterministic**: same verdict every run, or a pinned high reproduction rate for flaky bugs.
- **Fast**: seconds, not minutes.
- **Agent-runnable**: unattended, except for a structured HITL loop.
...
Shrink the repro to the smallest scenario that still goes red. Cut one variable at a time: inputs, callers, config, data, timing, environment, and steps. Re-run the loop after each cut and keep only what is load-bearing.

Done means every remaining element matters: removing any one element makes the loop go green or changes the symptom.
