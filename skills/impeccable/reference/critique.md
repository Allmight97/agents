...
2. **Compute the slug**:
   ```bash
   node "<impeccable-skill-dir>/scripts/critique-storage.mjs" slug "<resolved-path-or-url>"
   ```
...
4. If mutation is available, start `node "<impeccable-skill-dir>/scripts/live-server.mjs" --background`, present the browser if supported, label `[Human]`, scroll top, inject `http://localhost:PORT/detect.js`, wait 2-3 seconds, read `impeccable` console messages, then stop the live server.
