...
```bash
node "<impeccable-skill-dir>/scripts/live.mjs"
```
...
```
LOOP:
  node "<impeccable-skill-dir>/scripts/live-poll.mjs"   # default long timeout; no --timeout=
...
```
node "<impeccable-skill-dir>/scripts/live-poll.mjs" --stream   # stays running; one JSON line per event
...
```bash
node "<impeccable-skill-dir>/scripts/live-status.mjs"
node "<impeccable-skill-dir>/scripts/live-resume.mjs" --id SESSION_ID
node "<impeccable-skill-dir>/scripts/live-complete.mjs" --id SESSION_ID
```
...
```bash
node "<impeccable-skill-dir>/scripts/live-insert.mjs" --id EVENT_ID --count EVENT_COUNT --position after \
...
```bash
node "<impeccable-skill-dir>/scripts/live-wrap.mjs" --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
```bash
node "<impeccable-skill-dir>/scripts/live-poll.mjs" --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node "<impeccable-skill-dir>/scripts/live-poll.mjs" --reply EVENT_ID error "Short reason"
```
...
```bash
node "<impeccable-skill-dir>/scripts/live-poll.mjs" --reply EVENT_ID steer_done ["Optional short note for a browser toast"]
```
...
```bash
node "<impeccable-skill-dir>/scripts/detect-csp.mjs"
```
