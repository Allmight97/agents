...
description: Build, run, and debug macOS apps with shell-first Xcode and Swift workflows. Use when launching desktop apps or diagnosing macOS build, startup, or runtime failures.
---
...
For iOS or watchOS simulator/device work, use `ios-watch-substrate`. For
Xcode 27, SDK, runtime, destination, or `DEVELOPER_DIR` uncertainty, use
`xcode-27-toolchain` first.
...
1. Discover the project shape.
   ...
   - Confirm the target platform is macOS. If the user asks for iOS, iPadOS,
     watchOS, or a simulator/device run, switch to `ios-watch-substrate`.
...
- Toolchain check:
  - `DEVELOPER_DIR=<xcode-path> xcodebuild -version`
  - `DEVELOPER_DIR=<xcode-path> xcrun --sdk macosx --show-sdk-version`
- Scheme discovery:
...
- Do not convert the macOS `script/build_and_run.sh` contract into a simulator
  runner. Keep iOS and watchOS proof in `ios-watch-substrate`.
