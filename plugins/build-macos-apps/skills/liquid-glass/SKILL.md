---
name: liquid-glass
description: Implement and review Apple 27 SwiftUI Liquid Glass UI, especially macOS surfaces. Use when adopting system glass, removing conflicting custom chrome, or building glass surfaces.
---
...
For iOS and watchOS builds, keep the same posture: standard SwiftUI structure
first, custom glass second, and simulator/device proof through
`ios-watch-substrate`. Treat legacy compatibility flags such as
`UIDesignRequiresCompatibility` as audit clues, not as the primary fix for an
Apple 27 SDK build.
...
6. If the target is iOS or watchOS, verify the valid destination and runtime
   with `ios-watch-substrate`.
7. If the UI change also affects launch behavior for a SwiftPM GUI app, use
...
- Do not rely on `UIDesignRequiresCompatibility` as a forward-looking design
  fix for Apple 27 SDK work. Prefer native structure and verify current SDK
  behavior locally.
